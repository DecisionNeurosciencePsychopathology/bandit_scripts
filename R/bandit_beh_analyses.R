...
bdf = bdf %>% group_by(ID) %>%
  mutate(
    ...
    ## need chosen and max value from MFX, 11/27/17
...
lag_test <- bdf[,c(1:4,6,7:8,11:17,106,109,111:116)]
View(lag_test)

# test choice prediction by best value option
bm1 <-
  glmer(
    choice_numeric==3 ~ (best_value_option==3)*Group + (best_value_option==3)*age +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
# stay -- a(t)==a(t+1), repetition of a(t)
# correct_incorrect, reinf -- r(t), credited to a(t)
# value_A_stim, value_B_stim, value_C_stim -- v(t), before r(t), following r(t-1)
...
pdf(file = "bandit wins by group.pdf",
    width = 10,
    height = 6)
ggplot(na.omit(bdf), aes(x = Trial, y = correct_incorrect, color = Group)) + stat_smooth(method =
                                                                                           "loess") + theme_gray(base_size = 20) + ylab("Probability of winning")
dev.off()
...
cdf = melt(bdf[, c(1:37, 81:dim(bdf)[2])],
           na.rm = FALSE,
           measure.vars = c("choiceA", "choiceB", "choiceC"))
cdf$option <- cdf$variable
cdf$choice <- cdf$value
View(cdf)

# NB: change method to "loess" for publication
ggplot(na.omit(cdf[, c(1:24, 37:dim(cdf)[2])]), aes(x = Trial, y = choice, color = option)) + stat_smooth(method =
                                                                                                            "auto") + theme_gray(base_size = 20) + ylab("Choice probability")

ggplot(na.omit(cdf[, c(1:24, 37:dim(cdf)[2])]), aes(x = Trial, y = reinf_n, color = Group)) + stat_smooth(method =
                                                                                                            "auto") + theme_gray(base_size = 20) + ylab("Choice probability")

ggplot(na.omit(cdf[, c(1:24, 37:dim(cdf)[2])]), aes(x = Trial, y = choice, color = Group)) + stat_smooth(method =
                                                                                                           "auto") + theme_gray(base_size = 20) + ylab("Choice probability") +
...
ggplot(na.omit(bdf[, c(1:24, 81:dim(bdf)[2])]), aes(x = Trial, y = value_chosen, color = Group)) + stat_smooth(method =
                                                                                                                 "auto", alpha = 0.3) + theme_gray(base_size = 20) + ylab("Reward (behavioral study)")

ggplot(na.omit(bdf[, c(1:24, 81:dim(bdf)[2])]), aes(x = v_chosen_lag, y = stay, color = Group)) + geom_count(position = "jitter", size = 1) + theme_grey(base_size =
                                                                                                                                                           20) + ylab("Choice repeated?") + scale_color_brewer(palette = "Set1")
ggplot(na.omit(bdf[as.numeric(115$group1245) < 3,]), aes(x = v_chosen_lag, y =
                                                           stay, color = Group)) + geom_count(position = "jitter", size = 2) + theme_gray(base_size =
                                                                                                                                            20) + ylab("Choice repeated?") +  scale_fill_brewer(palette = "Spectral")

ggplot(na.omit(bdf[, c(1:17, 42:55)]), aes(x = value_chosen, y = stay_p, color = Group)) + stat_smooth(
  method = "glm",
  method.args = list(family = "binomial"),
  se = TRUE
) + theme_bw(base_size = 20) + labs(x = "Chosen value", y = "Probability of staying with the same choice")
...
sm1 <-
  glmer(
    stay ~ reinf_lag * stake_lag + stake + trial_scaled + stay_lag +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
# value predicts choice massively better than just win-stay/lose-switch
sm2 <- glmer(
  stay ~ v_chosen_lag * stake_lag + stake + trial_scaled +
    (1 | ID),
  family = binomial(),
  data = bdf,
  nAGQ = 0
...
# adding last reinforcement to long-term value improves the model significantly
sm3 <-
  glmer(
    stay ~ v_chosen_lag * stake_lag + reinf_lag * stake_lag + stake + trial_scaled +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
mm1 <-
  glmer(
    multinomial_choice ~ past_rew * stake_lag * trial_scaled + past_rew * I(trial_scaled ^
                                                                              2) + stake +
      age * trial_scaled + age * past_rew + Group * past_rew *
      trial_scaled + Group * past_rew * I(trial_scaled ^ 2) +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
ls_mm1 <-
  lsmeans(mm1,
          "trial_scaled",
          by = "Group",
          at = list(trial_scaled = c(-2, 0, 2)))
plot(ls_mm1, horiz = F)
# start looking at individual differences, starting with cognitive characteristics
im1 <-
  glmer(
    stay ~ reinf_lag * stake_lag + stake + trial_scaled + exit_scaled * reinf_lag +  iq_scaled *
      reinf_lag + group1245 * reinf_lag +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
im2 <-
  glmer(
    stay ~ past_rew * stake_lag + stake + trial_scaled + Group * past_rew + Group *
...
plot(ls_im2,
     ...
     horiz = F,
     ylab = "logit(probability of staying)",
     xlab = "Trial (early, middle, late in learning)")
...
ggplot(CLD, aes(
  ...
  labs(
    caption  = paste0(
      "A<C: z=3.1, p=0.002, A<D: z=1.7, p=0.087, A<I: z=2.7,  p=.005\n",
      "Boxes: LS mean.",
      "Error bars: 95% CI, \n"
    ),
    hjust = 0.5
...
ls_im2a <-
  lsmeans(im2,
          "trial_scaled",
          by = "past_rew",
          at = list(trial_scaled = c(-1.5, 0, 1.5)))
plot(ls_im2a,
     type ~ stay,
     horiz = F,
     ylab = "logit(probability of staying)",
...
vcheck1 <-
  lmer(value_max_vba_mfx ~ trial_scaled + Group + (1 |
                                                     ID), data = bdf)
summary(vcheck1)
car::Anova(vcheck1)
...
vcheck2 <-
  lmer(value_chosen_vba_mfx ~ trial_scaled + Group + (1 |
                                                        ID), data = bdf)
summary(vcheck2)
car::Anova(vcheck2)
...
vm1 <-
  glmer(
    stay ~ value_chosen_vba_mfx + stake_lag + stake + trial_scaled + stay_lag +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
vm1a <-
  glmer(
    stay ~ v_chosen_lag * stake_lag + v_chosen_lag * stake + trial_scaled + stay_lag +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
vm1b <-
  glmer(
    stay ~ v_chosen_lag * stake_lag + v_chosen_lag * stake + stay_lag +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
vm1c <-
  glmer(
    stay ~ v_chosen_lag * stake_lag * stake + trial_scaled + stay_lag +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
vm1d <-
  glmer(
    stay ~ v_chosen_lag * stake_lag * stake + v_chosen_lag * Group + trial_scaled + stay_lag +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
vm1r <-
  glmer(
    stay ~ v_chosen_lag * stake_lag * stake + v_chosen_lag * Group + trial_scaled *
      stay_lag +
      (1 + trial_scaled |
         ID),
    family = binomial(),
...
vm2 <-
  glmer(
    stay ~ stay_lag + stake + stake_lag + v_chosen_lag + trial_scaled + v_chosen_lag *
      stake_lag + stake + Group * v_chosen_lag + stay_lag +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
vm2_mfx <-
  glmer(
    stay ~ stay_lag + stake + stake_lag + value_chosen_vba_mfx_mc * Group + trial_scaled + stake_lag + stake  + stay_lag +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
ls_vm2_mfx <-
  lsmeans(vm2_mfx,
          "v_chosen_lag_mfx",
          by = "Group",
          at = list(v_chosen_lag_mfx = c(0.01, 0.99)))
plot(
  ls_vm2_mfx,
  type ~ stay,
  horiz = F,
...
vm2m <-
  glmer(
    stay ~ stay_lag + stake + stake_lag + v_max_lag + trial_scaled + v_max_lag *
      stake_lag + stake + Group * trial_scaled + Group * v_max_lag + stay_lag +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
vm2good <-
  glmer(
    stay ~ v_chosen_lag * stake_lag + stake + trial_scaled + Group * v_chosen_lag + age * v_chosen_lag +
      (1 |
         ID),
    family = binomial(),
    data = gdf,
...
vm2unquest <-
  glmer(
    stay ~ v_chosen_lag * stake_lag + stake + trial_scaled + Group * v_chosen_lag *
      trial_scaled +
      (1 |
         ID),
    family = binomial(),
...
vm2a <-
  glmer(
    stay ~ v_chosen_lag * stake_lag + stake + trial_scaled + Group * v_chosen_lag + Group *
      trial_scaled +
      (1 | ID),
    family = binomial(),
    data = bdf,
...
vm2a_mfx <-
  glmer(
    stay ~ v_chosen_lag_mfx * stake_lag + stake + trial_scaled + Group * v_chosen_lag_mfx + Group *
      trial_scaled +
      (1 | ID),
    family = binomial(),
    data = bdf,
...
vm3 <-
  glmer(
    stay ~ v_chosen_lag * stake_lag + stake + trial_scaled + Group * v_chosen_lag *
      trial_scaled + age * v_chosen_lag * trial_scaled +
      exit_scaled * v_chosen_lag * trial_scaled + iq_scaled * v_chosen_lag *
      trial_scaled + BIS_NONPLAN * v_chosen_lag * trial_scaled +
      (1 | ID),
    family = binomial(),
    data = bdf,
    nAGQ = 0
...
mfm1 <-
  glmer(
    stay ~ reinf_lag * stake_lag + stake + trial_scaled + Group * reinf_lag + Group *
      trial_scaled +
      (1 |
         ID),
    family = binomial(),
...
leastsquare = lsmeans(
  vm2a_mfx,
  pairwise ~ v_chosen_lag_mfx:Group,
  at = list(v_chosen_lag_mfx = c(0.1, 0.9)),
  adjust = "tukey"
)
CLD = cld(leastsquare,
          alpha = 0.05,
...
### Plot
pdf(file = "value on choice by group mfx.pdf",
    width = 8,
    height = 6)
pd = position_dodge(0.4)    ### How much to jitter the points on the plot
ggplot(CLD,
  ...
  labs(
    caption  = paste0(
      "A<C: z=4.7, A<D: z=5.7, A<I: z=3.8, all p<.001\n",
      "Boxes: LS mean.",
      "Error bars: 95% CI, \n",
...
rm1 <-
  glmer(
    reinf ~  Group * trial_scaled + Group * stake + Group * stake_lag +
      (1 |
         ID),
    family = binomial(),
    data = bdf,
...
bm1 <-
  glmer(
    multinomial_choice == "B" ~  Group * trial_scaled +
      (1 |
         ID),
    family = binomial(),
    data = postrev
...
ls_bm1 <-
  lsmeans(bm1,
          "trial_scaled",
          by = "Group",
          at = list(trial_scaled = c(0, 1, 2)))
plot(
  ls_bm1,
  type ~ response,
  horiz = F,
  ylab = "logit(probability of chosing B)",
  xlab = "Time after reversal"
)


# after reversal
rm2 <- glmer(
  reinf ~  Group * trial_scaled +
    (1 |
       ID),
  family = binomial(),
  data = postrev,
...
em1 <-
  glmer(
    multinomial_choice == "C" ~ reinf_lag * stake_lag + trial_scaled +
      (1 |
         ID),
    family = binomial(),
    data = prerevA,
...
car::Anova(em1)
ls_em1 <-
  lsmeans(em1, "reinf_lag", by = "stake_lag")
plot(
  ls_em1,
  type ~ response,
  horiz = F,
  ylab = "logit(p_C_choice)",
  xlab = "reinforcement"
)

em2 <-
  glmer(
    multinomial_choice == "B" ~ reinf_lag * stake_lag + trial_scaled +
      (1 |
         ID),
    family = binomial(),
    data = prerevA,
...
car::Anova(em2)
ls_em2 <-
  lsmeans(em2, "reinf_lag", by = "stake_lag")
plot(
  ls_em2,
  type ~ response,
  horiz = F,
  ylab = "logit(p_B_choice)",
  xlab = "reinforcement"
)
...
ggplot(na.omit(bdf[, c(1:24, 81:100)]), aes(x = h_lag, y = stay_p, color = v_chosen_lag)) + stat_smooth(
  method  = "glm",
  method.args = list(family = "binomial"),
  se = TRUE
)
ggplot(na.omit(bdf[, c(1:24, 81:100)]), aes(x = v_chosen_lag, y = stay_p, color = Group)) + stat_smooth(
  method  = "glm",
  method.args = list(family = "binomial"),
  se = TRUE
)

ggplot(na.omit(bdf[, c(1:24, 81:100)]), aes(x = h_lag, y = v_chosen_lag, color = stay)) + stat_smooth(
  method  = "glm",
  method.args = list(family = "binomial"),
  se = TRUE
)

ggplot(na.omit(bdf[, c(1:24, 81:100)]), aes(x = h_lag, y = v_chosen_lag, color = ID)) + geom_jitter() + facet_wrap(~
                                                                                                                     stay)

ggplot(na.omit(bdf[, c(1:24, 81:100)]), aes(x = h_lag, y = v_chosen_lag, color = stay)) + geom_jitter() + facet_wrap(~
                                                                                                                       ID + choice_lag)

ggplot(na.omit(bdf[, c(1:24, 81:123)]),
       aes(
         x = h_lag,
         y = v_max_lag,
         color = Trial,
         shape = stay
       )) + geom_jitter() + facet_wrap(~ ID + choice_lag)
bdf$value_chosen_vba_mfx_mc <-
  scale(bdf$value_chosen_vba_mfx)
bdf$H_vba_mfx_mc <- scale(bdf$H_vba_mfx)
bdf$v_chosen_lag_mfx_mc <-
  scale(bdf$v_chosen_lag_mfx)
...
ggplot(na.omit(bdf2[bdf2$ID == 217008, c(1:24, 81:125)]), aes(x = Trial, y = z, color = multinomial_choice)) + geom_jitter(size = 8) + facet_wrap(~
                                                                                                                                                    process, ncol = 1)
pdf(file = "value entropy choice by subject.pdf",
    width = 100,
    height = 100)
ggplot(na.omit(bdf2[, c(1:24, 81:125)]), aes(x = Trial, y = z, color = multinomial_choice)) + geom_jitter(size = .5) + facet_wrap(~
                                                                                                                                    ID + process)
dev.off()
...
# make sure 0-order effect of H on stay is negative before accounting for value
vhm00 <-
  glmer(
    stay ~ h_lag + stay_lag + stake + stake_lag +
      (1 |
         ID),
    family = binomial(),
    data = bdf,
...
plot(allEffects(vhm00))


vhm0 <-
  glmer(
    stay ~ v_chosen_lag + h_lag + stay_lag + stake + stake_lag +
      (1 |
         ID),
    family = binomial(),
    data = bdf,
...
# add current reinf, curent H, stake lead (at the following choice)
# reasonably complete model incorporating reinforcement and design factors
vhm1 <-
  glmer(
    stay ~  v_chosen_lag_mfx_mc * H_vba_mfx_mc + trial_scaled + stay_lag + stake_lead + stake*reinf +
      (1 |
         ID),
    family = binomial(),
    data = bdf,
...
summary(vhm1)
car::Anova(vhm1)
plot(allEffects(vhm1))
# allow group to interact with reinforcement; probably close to the actual truth
vhm2 <-
  glmer(
    stay ~  v_chosen_lag_mfx_mc * H_vba_mfx_mc * Group + reinf * Group + trial_scaled + stay_lag + stake_lead + stake +
      (1 |
         ID),
    family = binomial(),
    data = bdf,
...
summary(vhm2)
car::Anova(vhm2)
plot(allEffects(vhm2))


# this includes value of the chosen stimulus before the current outcome
vhm2mc_lag <-
  glmer(
    stay ~  v_chosen_lag_mfx_mc * h_lag_mfx_mc * Group + trial_scaled + stay_lag + stake_lag + stake +
      (1 |
         ID),
    family = binomial(),
    data = bdf,
...
summary(vhm2mc_lag)
car::Anova(vhm2mc_lag)

# incorporate the current outcome
vhm2mc_lag_r <-
  glmer(
    stay ~  v_chosen_lag_mfx_mc * h_lag_mfx_mc * Group + correct_incorrect *
      Group + trial_scaled + stay_lag + stake_lag + stake +
      (1 |
         ID),
    family = binomial(),
    data = bdf,
...
summary(vhm2mc_lag_r)
car::Anova(vhm2mc_lag_r)

# remove 3-way interaction, use current entropy
vhm2mc_lag_r_no3 <-
  glmer(
    stay ~  v_chosen_lag_mfx_mc * Group + v_chosen_lag_mfx_mc * H_vba_mfx_mc + H_vba_mfx_mc *
      Group + correct_incorrect * Group + trial_scaled + stay_lag + stake_lag + stake +
      (1 |
         ID),
    family = binomial(),
    data = bdf,
...
summary(vhm2mc_lag_r_no3)
car::Anova(vhm2mc_lag_r_no3)


fvhm1 <-
  glmer(
    stay ~  v_chosen_lag_f * h_lag_f + trial_scaled + stay_lag + stake_lag + stake +
      (1 |
         ID),
    family = binomial(),
    data = bdf,
...
vhm2 <-
  glmer(
    stay ~  v_chosen_lag * h_lag + v_chosen_lag * Group + h_lag * Group + trial_scaled + stay_lag + stake_lag + stake +
      (1 |
         ID),
    family = binomial(),
    data = bdf,
...
ls_vhm2 <-
  lsmeans(
    vhm2,
    "Group",
    by = c("v_chosen_lag", "h_lag"),
    at = list(
      v_chosen_lag = c(0, 0.5, 1),
      h_lag = c(1.408, 1.518, 1.585)
    )
  )
plot(ls_vhm2, horiz = F)

# add last reinforcement
vhm3 <-
  glmer(
    stay ~  v_chosen_lag * h_lag + v_chosen_lag * Group + h_lag * Group + reinf_lag *
      Group + trial_scaled + stay_lag + stake_lag + stake +
      (1 |
         ID),
    family = binomial(),
...
summary(vhm3)
car::Anova(vhm3)
plot(allEffects(vhm3))

vhm3_mfx <-
  glmer(
    stay ~  v_chosen_lag_mfx_mc * h_lag + v_chosen_lag_mfx_mc * Group + h_lag_mfx_mc *
      Group + reinf_lag * Group + trial_scaled + stay_lag + stake_lag + stake +
      (1 |
         ID),
    family = binomial(),
    data = bdf,
...
summary(vhm3_mfx)
car::Anova(vhm3_mfx)
plot(allEffects(vhm3_mfx))

cld(ls_vhm2)

fvhm2 <-
  glmer(
    stay ~  v_chosen_lag_f * Group + h_lag_f * Group +  v_chosen_lag_f * age_scaled + h_lag_f *
      age_scaled + trial_scaled + stay_lag + stake_lag + stake +
      (1 |
         ID),
    family = binomial(),
    data = bdf,
...
rpm1 <-
  manova(cbind(alpha_win, alpha_loss, decay, beta) ~ Group, data = beh_sub_df)
summary(rpm1)
anova(rpm1)
...
summary(rpm4)
anova(rpm4) #*
ggplot(data = beh_sub_df, aes(y = decay, x = Group, color = Group)) + geom_boxplot() + geom_jitter()
...
rprerevA <-
  subset(rdf, Trial < 150 & choice_lag == "A")
rpostrev <- subset(rdf, Trial > 150)

rem1 <-
  glmer(
    stay ~ reinf_lag * Group + trial_scaled * Group +
      (1 |
         ID),
    family = binomial(),
    data = rdf,
...
rem2 <-
  glmer(
    stay ~ past_rew * Group * trial_scaled + past_rew * age_scaled * trial_scaled +
      (1 |
         ID),
    family = binomial(),
    data = rdf,
...
leastsquare = lsmeans(rem2, pairwise ~ past_rew:Group, adjust =
                        "tukey")
CLD = cld(leastsquare,
          alpha = 0.05,
          Letters = letters,
...
lrem2 <-
  glmer(
    stay ~ past_rew * GroupLeth * trial_scaled + past_rew * age_scaled * trial_scaled +
      (1 |
         ID),
    family = binomial(),
    data = rdf,
...
leastsquare = lsmeans(lrem2, pairwise ~ past_rew:GroupLeth:trial_scaled, adjust =
                        "tukey")
CLD = cld(ls_lrem2,
          alpha = 0.05,
          Letters = letters,
...
rem2post <-
  glmer(
    stay ~ reinf_lag + trial_scaled + Group * reinf_lag + Group * trial_scaled + reinf_lag *
      BASELINEAGE + trial_scaled * BASELINEAGE +
      (1 |
         ID),
    family = binomial(),
    data = rpostrev,
...
rvm2 <-
  glmer(
    stay ~  Group * v_chosen_lag + Group * trial_scaled + stay_lag +
      (1 |
         ID),
    family = binomial(),
    data = rdf,
...
rvm3 <-
  glmer(
    stay ~  Group * v_chosen_lag + Group * trial_scaled +
      WTAR_SCALED_SCORE * v_chosen_lag + WTAR_SCALED_SCORE *
      v_chosen_lag +
      EXITtot * v_chosen_lag + EXITtot * trial_scaled +
      BIS_NONPLAN * v_chosen_lag + BIS_NONPLAN *
      trial_scaled +
      (1 |
         ID),
    family = binomial(),
...
xs[1] = xs[1] - .00005
rdf <-
  rdf %>% mutate(v_chosen_cat = cut(
    value_chosen,
    breaks = xs,
    labels = c("d1", "d2", "d3", "d4", "d5", "d6", "d7", "d8", "d9", "d10")
  ))
rdf <-
  rdf %>% mutate(v_chosen_cat_lag = cut(
    v_chosen_lag,
    breaks = xs,
    labels = c("d1", "d2", "d3", "d4", "d5", "d6", "d7", "d8", "d9", "d10")
  ))
...
rvmb <-
  glmer(
    stay ~  Group * v_chosen_cat_lag + Group * trial_scaled + stay_p +
...
car::Anova(rvmb)
ls_rvmb <-
  lsmeans(rvmb, "v_chosen_cat_lag", by = "Group")
plot(ls_rvmb,
     type ~ stay,
     horiz = F,
     ylab = "logit(probability of staying)",
...
leastsquare = lsmeans(rvmb, pairwise ~ v_chosen_cat_lag:Group, adjust =
                        "tukey")
CLD = cld(leastsquare,
          alpha = 0.05,
          Letters = letters,
...
ggplot(na.omit(rcdf[, c(2, 58, 65, 66)]), aes(x = Trial, y =
                                                choice, color = option)) + stat_smooth(method = "auto") + theme_bw(base_size =
                                                                                                                     20) + ylab("Choice probability") +
  scale_color_manual(values = c("blue", "red", "green"))

ggplot(na.omit(rcdf[, c(1:2, 57:66)]), aes(x = Trial, y = choice, color = Group)) + stat_smooth(method =
                                                                                                  "auto") + theme_bw(base_size = 20) + ylab("Choice probability") +
  facet_wrap(~ option)

ggplot(na.omit(rdf[, c(1:2, 47:65)]), aes(x = Trial, y = reinf_n, color = Group)) + stat_smooth(method =
                                                                                                  "auto") + theme_gray(base_size = 20) + ylab("Reward (replication)")

ggplot(na.omit(rdf[, c(1:9, 47:65)]), aes(x = Trial, y = value_chosen, color = Group)) + stat_smooth(method =
                                                                                                       "auto") + theme_gray(base_size = 20) + ylab("Chosen value (replication)")


ggplot(na.omit(rdf[, c(1:2, 47:68)]), aes(x = Trial, y = stay_p, color = Group)) + stat_smooth(method =
                                                                                                 "auto") + theme_bw(base_size = 20) + ylab("Probability of staying with the same choice") +
  facet_wrap(~ v_chosen_cat) #geom_jitter(alpha=0.2) +

ggplot(na.omit(rdf[, c(1:2, 47:66)]), aes(x = Trial, y = stay_p, color = v_chosen_cat)) + stat_smooth(method =
                                                                                                        "auto") + theme_bw(base_size = 20) + ylab("Probability of staying with the same choice") +
  facet_wrap(~ Group) #geom_jitter(alpha=0.2) +

ggplot(na.omit(rdf[, c(1:9, 47:66)]), aes(x = value_chosen, y =
                                            stay_p, color = Group)) + stat_smooth(method = "loess") + theme_bw(base_size =
                                                                                                                 20) + labs(x = "Chosen value", y = "Probability of staying with the same choice")

ggplot(na.omit(rdf[, c(1:9, 15, 47:67)]), aes(x = value_chosen, y =
                                                stay_p, color = GroupLeth)) + stat_smooth(
                                                  method = "gam",
                                                  method.args = list(family = "binomial"),
                                                  se = TRUE
                                                ) + theme_gray(base_size = 20) + labs(x = "Chosen value", y = "Probability of staying with the same choice")
ggplot(na.omit(rdf[, c(1:9, 15, 47:67)]), aes(x = Trial, y =
                                                stay_p, color = GroupLeth)) + stat_smooth(method = "loess") + theme_bw(base_size =
                                                                                                                         20) + labs(x = "Chosen value", y = "Probability of staying with the same choice")

ggplot(na.omit(rcdf[, c(1:2, 57:68)]),
       aes(
         x = Trial,
         y = choice,
         color = GroupLeth,
         linetype = option
       )) + stat_smooth(method = "auto") + theme_bw(base_size = 20) + ylab("Choice probability") +  facet_wrap(~
                                                                                                                 option, ncol = 2)
ggplot(na.omit(rcdf[, c(1:2, 57:68)]), aes(x = Trial, y = reinf_n, color = Group)) + stat_smooth(method =
                                                                                                   "auto") + theme_gray(base_size = 20) + ylab("Choice probability")


# plot trialwise stay probability
pdf(file = "attempters exploring rep.pdf",
    width = 10,
    height = 6)
ggplot(na.omit(rdf[, c(1:2, 47:65)]), aes(x = Trial, y = stay_p, color = Group)) + stat_smooth(method =
                                                                                                 "auto") + theme_bw(base_size = 20) + ylab("Probability of staying with the same choice") +
  facet_wrap(~ past_rew) #geom_jitter(alpha=0.2) +
dev.off()
...
rvhm2 <-
  glmer(
    stay ~  v_chosen_lag * h_lag + v_chosen_lag * Group + h_lag * Group + trial_scaled + stay_lag +
      (1 |
         ID),
    family = binomial(),
    data = rdf,
...
rfvhm2 <-
  glmer(
    stay ~  v_chosen_lag_f * Group + h_lag_f * Group +  v_chosen_lag_f * age_scaled + h_lag_f *
      age_scaled + trial_scaled + stay_lag +
      (1 |
         ID),
    family = binomial(),
    data = rdf,
...
cdf = melt(bdf[,c(1:37,81:100)], na.rm = FALSE, measure.vars = c("choiceA","choiceB","choiceC"))
cdf$option <- cdf$variable
cdf$choice <- cdf$value
View(cdf)

ggplot(na.omit(cdf[,c(1:24,37:58)]), aes(x=Trial, y=choice, color = option)) + stat_smooth(method="loess") + theme_gray(base_size=20) + ylab("Choice probability")

ggplot(na.omit(cdf[,c(1:24,37:58)]), aes(x=Trial, y=reinf_n, color = Group)) + stat_smooth(method="loess") + theme_gray(base_size=20) + ylab("Choice probability")

ggplot(na.omit(cdf[,c(1:24,37:58)]), aes(x=Trial, y=choice, color = Group)) + stat_smooth(method="loess") + theme_gray(base_size=20) + ylab("Choice probability") +
  facet_wrap(~option, ncol = 2) #geom_jitter(alpha=0.2) +

ggplot(na.omit(cdf[,c(1:24,37:58)]), aes(x=Trial, y=choice, color = option)) + stat_smooth(method="auto") + theme_gray(base_size=20) + ylab("Choice probability") +
  facet_wrap(~Group) #geom_jitter(alpha=0.2) +

ggplot(na.omit(bdf[,c(1:24,81:100)]), aes(x=Trial, y=value_chosen, color = Group)) + stat_smooth(method="loess", alpha=0.3) + theme_gray(base_size=20) + ylab("Reward (behavioral study)")

ggplot(na.omit(bdf[,c(1:24,81:100)]), aes(x=v_chosen_lag, y=stay, color = Group)) + geom_count(position = "jitter", size = 1) + theme_grey(base_size=20) + ylab("Choice repeated?") + scale_color_brewer(palette="Set1")
...
# adding last reinforcement to long-term value improves the model significantly
sm3 <- glmer(stay ~ v_chosen_lag*stake_lag + reinf_lag*stake_lag + stake + trial_scaled +
               (1|ID), family = binomial(), data = bdf, nAGQ = 0)
summary(sm3)
car::Anova(sm3)
...
rm1 <- glmer(reinf ~  Group*trial_scaled + Group*stake + Group*stake_lag +
             ...
             # do value and entropy have separately identifiable effects on choice?
             ggplot(na.omit(bdf[,c(1:24,81:100)]), aes(x = h_lag, y = stay_p, color = v_chosen_lag)) + stat_smooth(method  = "glm", method.args = list(family = "binomial"), se = TRUE)
             ggplot(na.omit(bdf[,c(1:24,81:100)]), aes(x = v_chosen_lag, y = stay_p, color = Group)) + stat_smooth(method  = "glm", method.args = list(family = "binomial"), se = TRUE)
             
             ggplot(na.omit(bdf[,c(1:24,81:100)]), aes(x = h_lag, y = v_chosen_lag, color = stay)) + stat_smooth(method  = "glm", method.args = list(family = "binomial"), se = TRUE)
             
             ggplot(na.omit(bdf[,c(1:24,81:100)]), aes(x = h_lag, y = v_chosen_lag, color = ID)) + geom_jitter() + facet_wrap(~stay)
             
             ggplot(na.omit(bdf[,c(1:24,81:100)]), aes(x = h_lag, y = v_chosen_lag, color = stay)) + geom_jitter() + facet_wrap(~ID + choice_lag)

             ggplot(na.omit(bdf[,c(1:24,81:100)]), aes(x = h_lag, y = v_max_lag, color = Trial, shape = stay)) + geom_jitter() + facet_wrap(~ID + choice_lag)
             ...
             # make sure 0-order effect of H on stay is negative before accounting for value
             vhm00 <- glmer(stay ~ h_lag + stay_lag + stake + stake_lag +
                             (1|ID), family = binomial(), data = bdf, nAGQ = 0)
             plot(allEffects(vhm00))
             ...
             # add last reinforcement 
             vhm3 <- glmer(stay ~  v_chosen_lag*h_lag + v_chosen_lag*Group + h_lag*Group + reinf_lag*Group + trial_scaled + stay_lag + stake_lag + stake +
                             (1|ID), family = binomial(), data = bdf, nAGQ = 0)
             summary(vhm3)
             car::Anova(vhm3)
             ...
             rdf$stay_lag <- as.factor(rdf$stay_lag)
             rvhm3 <- glmer(stay ~  v_chosen_lag*h_lag + v_chosen_lag*Group + h_lag*Group + reinf_lag*Group + trial_scaled + stay_lag +
                              (1|ID), family = binomial(), data = rdf, nAGQ = 0)
             summary(rvhm3)
             car::Anova(rvhm3)
             plot(allEffects(rvhm3))
...
library(readr)
trial_df <- read_csv("R/bandit_df1.csv")
View(trial_df)
sub_df <- read_csv("R/bandit_df2.csv")
View(sub_df)
sub_df$group1245 <- as.factor(sub_df$group1245)
sub_df$group12467 <- as.factor(sub_df$group12467)
