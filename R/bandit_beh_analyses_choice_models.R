...
s22_reinf <- update(s11_reinf,data = gdf)
summary(s22_reinf)
...
sm3g <-   glmer(
  stay ~ stake + stay_lag +
    value_max_vba_mfx * reinf * I(scale(v_ch_diff)) * Group  +
    (stay_lag | ID),
  family = binomial(),
  data = gdf, #InstEval,
  #   glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)),
...
sm3g <-   glmer(
  stay ~ stake + stay_lag +
    value_max_vba_mfx * Group +  reinf * Group +  I(scale(v_ch_diff)) * Group  +
    (stay_lag | ID),
  family = binomial(),
  data = gdf, #InstEval,
  #   glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 100000)),
...
# v_chosen_lag -- value of a(t),  v(t)
# v_chosen_lag_updated -- value of a(t) following r(t), v(a_t,t+1)
# v_ch_diff = v_chosen_lag_mfx - v_max_lag_mfx -- whether current choice was exploitative (0) or exploratory (<0)


#  do attempters switch away from high-value choices? (exploring win-switches)
vm1 <-   lmerTest::lmer(value_chosen_vba_mfx ~ stake + stay_lag + 
    stay*Group + reinf * Group +
    value_max_vba_mfx * Group  + I(scale(v_ch_diff)) * Group  +
    (1 | ID),
  data = gdf)
summary(vm1)
car::Anova(vm1, type = 'III')

# replicate
r_vm1 <-   lmerTest::lmer(value_chosen_vba_mfx ~ stay_lag + 
                          stay*Group + reinf * Group +
                          value_max_vba_mfx * Group  + I(scale(v_ch_diff)) * Group  +
                          (1 | ID),
                        data = rdf)
...
sr_vm1 <-   lmerTest::lmer(value_chosen_vba_mfx ~ stay_lag + 
                            stay*Group + reinf * Group +
                            value_max_vba_mfx * Group  + I(scale(v_ch_diff)) * Group  +
                            (1 | ID),
                          data = sdf)
...
# refit for stargazer, simplify
vm1star <- lme4::lmer(value_chosen_vba_mfx ~ stake + stay_lag + reinf +
                          stay* reinf * value_max_vba_mfx *Group +
                          (stay + reinf  | ID),
                        data = gdf)

r_vm1star <- lme4::lmer(value_chosen_vba_mfx ~ stay_lag + reinf +
                          stay* reinf * value_max_vba_mfx *Group +
                          (stay + reinf  | ID),
                          data = rdf)

sr_vm1star <- lme4::lmer(value_chosen_vba_mfx ~ stay_lag + reinf +
                           stay* reinf * value_max_vba_mfx *Group +
                           (stay + reinf  | ID),
                           data = sdf)
stargazer(vm1star, sr_vm1star, r_vm1star, type="html", out="chosen_value_replication.htm", digits = 2,single.row=TRUE,omit.stat = "bic",
          column.labels = c("Sample 1/fMRI", "Sample 1/behavioral", "Sample 2/behavioral"),
          star.cutoffs = c(0.05, 0.01, 0.001))

# plot 4-way interactions
lsm <- lsmeans::lsmeans(vm1star, "value_max_vba_mfx",by = c("stay","reinf","Group"), at = list(value_max_vba_mfx = c(0,1)))
plot(lsm, horiz = F)
lsm <- lsmeans::lsmeans(r_vm1star, "value_max_vba_mfx",by = c("stay","reinf","Group"), at = list(value_max_vba_mfx = c(0,1)))
plot(lsm, horiz = F)
lsm <- lsmeans::lsmeans(sr_vm1star, "value_max_vba_mfx",by = c("stay","reinf","Group"), at = list(value_max_vba_mfx = c(0,1)))
plot(lsm, horiz = F)


# plot 3-way interactions
lsm <- lsmeans::lsmeans(vm1star, "reinf",by = c("stay","Group"))
plot(lsm, horiz = F)
lsm <- lsmeans::lsmeans(r_vm1star, "reinf",by = c("stay","Group"))
plot(lsm, horiz = F)
lsm <- lsmeans::lsmeans(sr_vm1star, "reinf",by = c("stay","Group"))
plot(lsm, horiz = F)
