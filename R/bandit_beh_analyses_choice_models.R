...
sm3gm <-   glmer(
  stay ~ stake + trial_scaled + v_max_lag_mfx * stay_lag *Group  + reinf * stay_lag * Group  +
    (1 | ID),
  family = binomial(),
  data = gdf,
  nAGQ = 0)
...
stargazer(sm3, sm3g, sm3gm, type="html", out="sm3g.htm", digits = 2,single.row=TRUE, star.cutoffs = c(0.05, 0.01, 0.001))
