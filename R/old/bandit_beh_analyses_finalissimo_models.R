...
rt_g2 <- lmerTest::lmer(log(RT)  ~ log(RT_lag) +
                          ...
                          stay_lag*abs(PE_chosen_vba_lag)*Group + 
                          (1 | ID),
                        data = gdf[gdf$RT>0 & gdf$RT<4000,])
summary(rt_g2)
...
# anova(rt_g1,rt_g2)
rt_g2star <- lme4::lmer(log(RT)  ~ log(RT_lag) +
                          reinf_lag*Group + stake_lag*Group + stay_lag*Group + trial_scaled + 
                          stay_lag*v_max_lag_mfx*Group + stay_lag*v_ch_diff*Group + 
                          stay_lag*abs(PE_chosen_vba_lag)*Group + 
                          (1 | ID),
                        data = gdf[gdf$RT>0 & gdf$RT<4000,])
stargazer(rt_g2star, type="html", out="rt_g2star.htm", digits = 2,single.row=TRUE, star.cutoffs = c(0.05, 0.01, 0.001))
...
# add interaction with stay/switch
rrt_g2 <- lmerTest::lmer(log(RT)  ~ log(RT_lag) + 
                           reinf_lag*Group + stay_lag*Group + trial_scaled + 
                           stay_lag*v_max_lag_mfx*Group + stay_lag*v_ch_diff*Group + 
                           stay_lag*abs(PE_chosen_vba_lag)*Group + 
                           (1 | ID),
                         data = rdf[rdf$RT>0 & rdf$RT<6000,])
...
rrt_g2star <- lme4::lmer(log(RT)  ~ log(RT_lag) + 
                           reinf_lag*Group + stay_lag*Group + trial_scaled + 
                           stay_lag*v_max_lag_mfx*Group + stay_lag*v_ch_diff*Group + 
                           stay_lag*abs(PE_chosen_vba_lag)*Group + 
                           (1 | ID),
                         data = rdf[rdf$RT>0 & rdf$RT<6000,])
stargazer(rrt_g2star, type="html", out="rrt_g2star.htm", digits = 2,single.row=TRUE, star.cutoffs = c(0.05, 0.01, 0.001))
